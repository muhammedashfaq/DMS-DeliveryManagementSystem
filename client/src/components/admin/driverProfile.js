...
const Driverdetails = () => {
  ...
  const getprofile = async () => {
    try {
      ...
      const response = await axios.post(`/admin/get-profile/${id}`);
      dispatch(hideloading());

      console.log("resdddd,", response.data.data);
  ...
  const clickupdate=async(driverid,status)=>{
    try {
        dispatch(showloading())
      const response = await axios.put(`/admin/driverstatusUpdat/${driverid}`,{status:status})
      dispatch(hideloading())
      if(response.data.success){
        toast.success(response.data.message)
       getprofile();
          ...
          <div className="flex flex-col max-w-md p-6 dark:bg-gray-900 dark:text-gray-100 mt-10 ml-10">
            ...
            <div className="flex justify-evenly">
              <Popover placement="bottom">
                <PopoverHandler>
                  <Button className={`bg-gray-700 mt-3 ${driver?.activestatus==="Terminated" ?`bg-gray-800 `:driver?.activestatus==="Blocked" ?`bg-red-800`:`bg-blue-800` }`}>{driver?.activestatus}</Button>
                </PopoverHandler>
                <PopoverContent className="w-40">
                  <List className="p-0">
               
                  <a href="#" className="text-initial" onClick={()=>{clickupdate(driver?._id,"Active")}}>
                      <ListItem>Active</ListItem>
                    </a>
                    <a href="#" className="text-initial" onClick={()=>{clickupdate(driver?._id,"Blocked")}}>
                      <ListItem>Block</ListItem>
                    </a>
                    <a href="#" className="text-initial"onClick={()=>{clickupdate(driver?._id,"Terminated")}}>
        ...
        <div className="col-span-2 bg-gray-900 m-10 h-max dark:text-gray-100">
          <div className="flex justify-around dark:bg-gray-800  ">
            <a
              ...
              href="#"
              className={`px-5 py-1  dark:border-gray-700 ${tabs==="about" && `border-b-2`}`}
            ...
            <a
              rel="noopener noreferrer"
              href="#"
              className={`px-5 py-1  dark:border-gray-700 ${tabs==="jobdescription" && `border-b-2`}`}
